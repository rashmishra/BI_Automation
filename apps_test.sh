...
from (SELECT
  NVL(p.order_id,ol.orderid) AS order_id_new,
  p.* from
(SELECT
  INTEGER(RTRIM(LTRIM(REGEXP_EXTRACT(REGEXP_EXTRACT(event_value,'(\"order id\":\"[0-9]+\")'),'(\"[0-9]+\")'),'\"'),'\"')) order_id, 
  INTEGER(RTRIM(LTRIM(REGEXP_EXTRACT(REGEXP_EXTRACT(event_value,'(\"transaction id\":\"[0-9]+\")'),'(\"[0-9]+\")'),'\"'),'\"')) transaction_id
...
tableName=appsflyer_processed_table_test
...
echo "Appsflyer table status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com 
