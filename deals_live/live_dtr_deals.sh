...
v_query_live_dtr_deals="SELECT 
 ...
 FROM FLATTEN(FLATTEN(FLATTEN(FLATTEN(FLATTEN(FLATTEN([big-query-1233:Atom.nile] , merchant.skus), offers)
                ...
                inner join  (SELECT _id as deal_id, units.dtitle as deal_title FROM [big-query-1233:Atom.deal] 
where (units.dtitle like 'XMAS%2016%' or units.dtitle like 'NYE%2017%') ) d on a._id = d.deal_id
                  where 
                     a.offers.isActive = TRUE and a.isActive = TRUE
