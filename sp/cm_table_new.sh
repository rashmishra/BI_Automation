...
SELECT
  ...
  left outer join BI_Automation.sales_rep_mapping as m on m.sales_rep = D.deal_owner 
...
tableName=temp_daily_cm_dont_delete
...
tableName=daily_reports_table_cm_new
v_destination_tbl="$v_dataset_name.${tableName}";
...
echo "CM table status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com ashish.singh@nearbuy.com    ## akhil.dhingra@nearbuy.com alka.gupta@nearbuy.com
