...
v_query_CM_table_yesterday="select * from BI_Automation.daily_reports_table_CM where date(createdAt) = date(STRFTIME_UTC_USEC(now()-86400000000, '%Y-%m-%d'))"
##echo -e "Query: \n $v_query_CM_table_yesterday";

tableName=CM_table_yesterday
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_CM_table_yesterday\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_CM_table_yesterday" &
v_first_pid=$!
...
from TABLE_DATE_RANGE  ([108795712.ga_sessions_],TIMESTAMP(date(STRFTIME_UTC_USEC(now()-86400000000*1, '%Y-%m-%d'))),TIMESTAMP(date(STRFTIME_UTC_USEC(now()-86400000000*1, '%Y-%m-%d')))) 
...
tableName=ga_orderid_table_yesterday
v_destination_tbl="$v_dataset_name.${tableName}";
...
v_query_ga_alldata_table="SELECT  a.orderId AS orderId
...
a.totalcashback as totalcashback  
 ...
 LEFT JOIN  
 (select * from BI_Automation.appslyer_processed_table_3 where rank1 = 1) as ap on a.orderId = ap.a_order_id_new 
 LEFT JOIN (
  SELECT
    dcg AS dcg_ga
    ,campaign_grouping 
 ...
 is_reengagement,totalcashback"
##echo -e "Query: \n $v_query_ga_alldata_table";

tableName=ga_alldata_table
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --append -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_alldata_table\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --append -n 0 --destination_table=$v_destination_tbl "$v_query_ga_alldata_table" &
##v_first_pid=$!
v_downs_rpt_pids+=" $!"
...
v_query_ga_alldata_table_new="SELECT  a.orderId AS orderId
  , a.vertIcal AS vertIcal
  , a.userId AS customerid
  , a.createdAt AS createdat
  , a.TransactionValue AS TransactionValue
...
tableName=ga_alldata_table_new 
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --append -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_alldata_table_new\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --append -n 0 --destination_table=$v_destination_tbl "$v_query_ga_alldata_table_new" &
v_downs_rpt_pids+=" $!"
...
v_query_ga_alldata_table_yesterday="SELECT  a.orderId AS orderId
  , a.vertIcal AS vertIcal
  , a.userId AS customerid
  , a.createdAt AS createdat
  , a.TransactionValue AS TransactionValue
...
tableName=ga_alldata_table_yesterday
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_alldata_table_yesterday\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_ga_alldata_table_yesterday" &
v_downs_rpt_pids+=" $!"
...
v_query_ga_downstream_table="SELECT 
  r.*, 
  v.* 
FROM 
(select 
...
totalcashback
from 
  BI_Automation.ga_alldata_table_yesterday )
r 
LEFT OUTER JOIN (
  SELECT
  ...
  FROM
    BI_Automation.ga_alldata_table_new
...
tableName=ga_downstream_table
v_destination_tbl="bi.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --append -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_downstream_table\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --append -n 0 --destination_table=$v_destination_tbl "$v_query_ga_downstream_table" &
v_downs_rpt_pids+=" $!"

wait $!;

# ga_downstream_table loading. Replace existing
v_query_ga_downstream_table2="SELECT a.*, b.offerid  FROM [big-query-1233:bi.ga_downstream_table] a
left join (select string(Deal_Id) as offerid from [big-query-1233:dbdev.live_dtr_deals]) b on a.r_offerId = b.offerid"

tableName=ga_downstream_table2
v_destination_tbl="bi.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_downstream_table2\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_ga_downstream_table2" &
v_downs_rpt_pids+=" $!"
...
echo "Downstream all table status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com  alka.gupta@nearbuy.com
