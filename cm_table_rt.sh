...
v_query_temp_cm_rt="SELECT
  ...
  sum(ol.cashbackamount/100.0) as totalcashback
  FROM  Atom_rt.order_header AS oh 
  LEFT OUTER JOIN Atom_rt.order_line AS ol ON  ol.orderid = oh.orderid 
...
tableName=cm_table_rt
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_cm_table_rt\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_cm_table_rt" &
v_second_pid=$!
v_CM_Tbl_pids+=" $!"
...
# daily_reports_table_CM loading. Replace existing
v_query_CM_table_rt_with_dtr="SELECT a.*, case when b.deal_id is null then 'BAU' else 'DTR' end Deal_Type,d.dealtitle as dealtitle FROM [big-query-1233:BI_Automation.cm_table_rt] a
...
tableName=CM_table_rt_with_dtr
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_CM_table_rt_with_dtr\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_CM_table_rt_with_dtr" &
##v_first_pid=$!
v_CM_Tbl_pids+=" $!"
...
echo "CM table Real Time status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com ##akhil.dhingra@nearbuy.com alka.gupta@nearbuy.com
