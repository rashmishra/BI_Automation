...
v_query_appsflyer_reengagement_table="select a.*,rank() over (partition by a.order_id_new order by ap.install_time asc) rank1, 
case when y.count > 1 then 'Reattributed' else 'na' end as reaatributed_flag from
(SELECT
  COALESCE(ap.order_id,ol.orderid,ap.af_content_id) AS order_id_new,
  ap.* from
(SELECT
  imei,
  customer_user_id,
...
from (SELECT
  COALESCE(p.order_id,ol.orderid,p.af_content_id) AS order_id_new,
  p.* from
(SELECT
  INTEGER(RTRIM(LTRIM(REGEXP_EXTRACT(REGEXP_EXTRACT(event_value,'(\"order id\":\"[0-9]+\")'),'(\"[0-9]+\")'),'\"'),'\"')) order_id, 
  INTEGER(RTRIM(LTRIM(REGEXP_EXTRACT(REGEXP_EXTRACT(event_value,'(\"transaction id\":\"[0-9]+\")'),'(\"[0-9]+\")'),'\"'),'\"')) transaction_id
