...
v_query_CM_table_full="select 
orderId, vertIcal, userId, createdAt, TransactionValue, priceAfterPromo, promoCode ,
creditPoints, orderStatus, SOURCE, cda, dealid, offerId, categoryid, ol_offertitle, merchantName, merchantId, transactionId, paymentFlag, paymentGatewayReference, NumberofVouchers, GR, payable_by_PG, deal_owner, m_location, m_manager, isnew, p_LPD, p_FPD, activation_cost1, txn, buyinglat, buyinglong, buyingcity, buyingstate, Promo
,b.offerproductid as offerproductid
from BI_Automation.daily_reports_table_CM  a
inner join (select string(offers.key) as offers_id, offers.units.odesc.skus.productId as offerproductid, 
offers.units.odesc.skus.inclusions.title as offerskutitle  
from  flatten([big-query-1233:Atom.offer],offers.key))  b on b.offers_id = a.offerid
where  not date(createdAt) = date(STRFTIME_UTC_USEC(now(), '%Y-%m-%d'))"
...
v_query_ga_alldata_table="SELECT  a.orderId AS orderId
  ...
  , a.offerproductid as offerproductid
  ,1 as rank1
  ,y.dcg_ga AS dcg_GA
 ...
 FROM 
 temp.CM_table_full a
 LEFT JOIN  
 (select * from temp.appslyer_processed_table2 where rank1 = 1) as ap on a.orderId = ap.a_order_id_new 
 LEFT JOIN (
  SELECT
    dcg AS dcg_ga
    ,campaign_grouping 
  ...
  FROM temp.ga_orderid_table_full
...
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_alldata_table\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_ga_alldata_table" &
##v_first_pid=$!
v_downs_rpt_pids+=" $!"
...
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_alldata_table_new\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_ga_alldata_table_new" &
v_downs_rpt_pids+=" $!"
...
v_destination_tbl="temp.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_ga_downstream_table\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_ga_downstream_table" &
v_downs_rpt_pids+=" $!"
...
echo "Downstream all table status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com ## akhil.dhingra@nearbuy.com alka.gupta@nearbuy.com
