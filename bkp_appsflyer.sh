...
tableName=appsflyer_processed_table
...
v_query_appslyer_processed_table_2="SELECT
  NVL(ap.order_id,ol.orderid) AS order_id_new,
  ap.*
FROM  [BI_Automation.appsflyer_processed_table] ap
LEFT JOIN
...
tableName=appslyer_processed_table_2
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl "$v_query_appslyer_processed_table_2""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_appslyer_processed_table_2" &
v_second_pid=$!
v_Apps_tbl_pids+=" $!"
...
# appslyer_processed_table_2 loading. Replace existing
v_query_appslyer_processed_table_3="select a.*,rank() over (partition by a.order_id_new order by ap_install_time asc) rank1, 
case when y.count > 1 then 'Reattributed' else 'na' end as reaatributed_flag
from BI_Automation.appslyer_processed_table_2 a
Left outer join
(select order_id_new, count(*) as count from BI_Automation.appslyer_processed_table_2 group by 1) y
On a.order_id_new = y.order_id_new"
##echo -e "Query: n $v_query_CM_table_yesterday";

tableName=appslyer_processed_table_3
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl "$v_query_appslyer_processed_table_3""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_appslyer_processed_table_3" &
##v_first_pid=$!
v_Apps_tbl_pids+=" $!"
...
echo "Appsflyer table status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com akhil.dhingra@nearbuy.com alka.gupta@nearbuy.com
