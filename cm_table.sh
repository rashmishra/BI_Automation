...
v_query_temp_CM_Table="SELECT
  ...
  buyinglat, buyinglong, l.city as buyingcity, l.state as buyingstate, promo.promocode as Promo, sum(ol.cashbackamount/100.0) as Totalcashback,d.dealtitle as dealtitle
  ...
  LEFT OUTER JOIN  (select string(_id) as dealid, dealOwner as deal_owner,units.dtitle as dealtitle from Atom.deal) D on D.dealid = ol.dealid 
 ...
 GROUP BY orderId, vertIcal, userId, oh.createdat,oh.promocode,ol.offerid,creditPoints,payable_by_PG, orderStatus,SOURCE,cda,dealid,offerId,categoryid,ol.offertitle,merchantName,createdAt,promoCode,merchantId,transactionId,paymentFlag, paymentGatewayReference,deal_owner,m.location,isnew,oh.creditsRequested,T.payableAmount,w.activation_cost,ol.marginPercentage, 
  OL.flatcommission,m.manager,w.offer_correct,w.marginGR,p.LPD, p.FPD,oh.userid,oh.orderid,  buyinglat, buyinglong, buyingcity, buyingstate, promo,dealtitle"
...
v_query_daily_reports_table_CM="SELECT
  ...
  buyinglat, 
  buyinglong, 
  buyingcity,buyingstate,Promo,Totalcashback, dealtitle
FROM
...
# daily_reports_table_CM loading. Replace existing
v_query_daily_report_table_with_dtr="SELECT a.*, case when b.deal_id is null then 'BAU' else 'DTR' end Deal_Type,d.dealtitle as dealtitle FROM [big-query-1233:BI_Automation.cm_table_rt] a
left join (select string(deal_id) as deal_id from [big-query-1233:dbdev.dtr_deals_live]) b on a.dealid = b.deal_id
left join (select string(_id) as dealid, units.dtitle as dealtitle from [big-query-1233:Atom.deal] ) d on d.dealid = a.dealid"

tableName=daily_report_table_with_dtr
v_destination_tbl="$v_dataset_name.${tableName}";
echo "bq query --maximum_billing_tier 100 --allow_large_results=1  --replace -n 1 --destination_table=$v_destination_tbl \"$v_query_bi_daily_report_table_with_dtr\""
bq query --maximum_billing_tier 100 --allow_large_results=1 --replace -n 0 --destination_table=$v_destination_tbl "$v_query_daily_report_table_with_dtr" &
##v_first_pid=$!
v_CM_Tbl_pids+=" $!"
...
echo "CM table status:$v_table_status`date`" | mail -s "$v_table_status" rashmi.mishra@nearbuy.com alka.gupta@nearbuy.com
